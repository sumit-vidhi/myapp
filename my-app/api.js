...
function addslashes_string(str) {
                          ...
                          //  //console.log(values);
                            ...
                            if(role==1){
                                ...
                                userModel.query("Insert  into users_details ("+field+") OUTPUT Inserted.ID values("+values+")", function(err, rows) {
                                    ...
                                    newUser.first_name = addslashes(post.first_name);
                                        newUser.last_name = addslashes(post.last_name);
                            ...
                            if(role==2){
                                ...
                                var field="trainer_id,first_name,last_name,nick_name,state,birth_date,phone_number,perferred_languages,additional_language,specialities,"+
                                ...
                                var values="'"+user_id+"','"+addslashes(post.first_name)+"','"+addslashes(post.last_name)+"','"+addslashes(post.nickname)+"','"+addslashes(post.state)+"','"+post.birth_date+"','"+post.phone_number+"','"+post.preferred_language+"',"+
                        ...
                        userModel.query(q,function(err, urows) {
                            if(err) {
                               res.send({ code: 100,message: 'Password not updated. Please try again'});
                                ...
                                newUser={};
                                newUser.name = addslashes(rows.recordset[0].name);
                                newUser.email = post.email;
                                newUser.code=rePass;
                ...
                userModel.query(qd,function(err, rows) {
                    ...
                    else 
                    {
                        ...
                        if(err) {
                            ...
                            name=addslashes(trainerrows.recordset[0].name);
                            newUser={};    
                            newUser.name = addslashes(name);
                      ...
                      userModel.query(qd,function(err, userrows) {
                        ...
                        userModel.query("select u.id,u.name,u.email,us.email as fromemail,u.photo,us.name as fromname,ts.zip as zip,ts.city as city,ts.street as street,ts.cvr_vat as cvr_vat from trainers_hire th left join users u  on th.user_id=u.id left join users us on th.trainer_id=us.id left join trainers_details ts on ts.trainer_id=us.id  where    th.user_id="+userId+" and th.trainer_id="+trainerId, function(err, rows) {
                            if (rows.recordset.length<1) {                   
                                ...
                                newUser.bytrainer= addslashes(rows.recordset[0].fromname);
                 ...
                 res.send({ code: 100,message: 'A valid user is required'});                  
                           ...
                           var q = "UPDATE trainers_hire SET status = '1' WHERE id='" +userId+"'";   
                               ...
                               if(err) {
                                ...
                                newUser.name = addslashes(rows.recordset[0].name);
                                newUser.email = rows.recordset[0].email;
                                newUser.message="Your request has been approved by "+addslashes(rows.recordset[0].fromname);
                                newUser.bytrainer= addslashes(rows.recordset[0].fromname);
                     ...
                     res.send({ code: 100,message: 'A valid user is required'});                  
                               ...
                               var q = "delete from  trainers_hire  WHERE id='" +userId+"'";   
                                   ...
                                   if(err) {
                                    ...
                                    newUser.name = addslashes(rows.recordset[0].name);
                                    newUser.email = rows.recordset[0].email;
                                    newUser.message="Your request has been rejected by "+addslashes(rows.recordset[0].fromname);
                                    newUser.bytrainer= addslashes(rows.recordset[0].fromname);
                                    newUser.photo= rows.recordset[0].photo;
                    ...
                    else    
                    {
                       ...
                       console.log(we);
                        ...
                        _update_subscription=function(id,trid){
                            ...
                            qs: { amount:amount,continue_url: 'http://52.169.138.196:3001/payment_status/'+trid+'/'+id},
        ...
        if(accessToken!=='') { 
                ...
                userModel.query(qd,function(err, rows) {
                    ...
                    else 
                    {
                        id=rows.recordset[0].id;
                        orderrows22=[];
                        userModel.query("select us.name,us.photo,us.email,u.id as id,trainers_hire.trainer_id as trainerId  from trainers_hire  left join users u on u.id=trainers_hire.user_id left join users us on us.id=trainers_hire.trainer_id  where trainers_hire.status=1 and trainers_hire.user_id="+id, function(err, orderrows) {
                                   ...
                                   if(body.state=='cancelled'){
                                       userModel.query("update orders set status='cancel' where subscription_id="+user_Id, function(err, orderrows) {
                                           if(err) {
                                               ...
                                               newUser={};
                                               newUser.trainername = addslashes(name);
                                               newUser.email =email;
                                               newUser.name= rows.recordset[0].name;
