                ...
                userModel.query("select * from users where  email='"+username+"'", function(err, rows) {
                    if(err) {
                        ...
                        if (rows.recordset.length>0) {                     
                            ...
                            var field="name,email,password,auth_token,confirmation_token,facebook_id,confirmed_at,role,photo,is_loggedin,created_at";
                            var values="'"+name+"','"+username+"','"+password+"','"+auth_token+"','"+confirm_token+"','"+facebook_id+"','"+confirmed_at+"','"+role+"','"+photo+"','"+is_loggedin+"','"+mysqlTimestamp+"'";
                          //  //console.log(values);
                            ...
                            if(role==2){
                                ...
                                "education,certifications,description,addtional_description,city,street,country,zip,weeklyprice";
                                var values="'"+user_id+"','"+post.first_name+"','"+post.last_name+"','"+post.nickname+"','"+post.birth_date+"','"+post.phone_number+"','"+post.preferred_language+"',"+
                                "'"+post.second_language+"','"+post.specialities+"','"+post.educations+"','"+post.certifications+"','"+post.short_description+"','"+post.description+"',"+
                                "'"+post.city+"','"+post.street+"','"+post.country+"','"+post.zip+"','"+post.price_week+"'";
   ...
   get_user_details:function(req, res, next){ 
    ...
    if(accessToken) {
        if(accessToken!=='') { 
                ...
                var qd = "SELECT (select count(*) from trainers_hire where (trainer_id="+userId+" or user_id="+userId+") and status=1) as hiredata,u.id,u.photo,u.email,u.role,"+fields+" FROM users  u left join "+table+" on "+refer+"."+key+"=u.id WHERE  u.auth_token='"+accessToken+"'";                            
   ...
   get_trainers_listing:function(req, res, next){ 
    ...
    //console.log(post);
        if(post) { 
            ...
            if(post.location!="" && post.location){
             ...
             if(post.rating!="" && post.rating){
                term=post.rating;
               // wildcard="'%"+term+"%'";
                q+="and (select avg(rating) from trainers_reviews where td.trainer_id=trainers_reviews.trainer_id)="+term;
             }
                 userModel = new MyAppModel();
                 if(accessToken){
                 var qd = "SELECT role,id FROM users  WHERE auth_token='"+accessToken+"'";                            
     ...
     userModel.query(qd,function(err, totalrows) {
                 ...
                 var qd = "SELECT u.id,u.photo,u.email,u.role,(select avg(rating) from trainers_reviews where td.trainer_id=trainers_reviews.trainer_id) as rating,u.name,"+fields+" FROM users  u left join "+table+" on "+refer+"."+key+"=u.id where u.role=2  "+q+" order by "+orderby+" OFFSET "+page+" ROWS FETCH NEXT 3   ROWS ONLY";                            
   ...
   get_hire_trainer_user:function(req,res,next){
    let accessToken = req.get('Authorization');
        if(accessToken) { 
                ...
                var qd = "SELECT us.id,us.auth_token as token,us.photo,us.email,us.is_loggedin as isOnline,us.role,us.name,(select message from messages where recipient_id="+rows.recordset[0].id+" order by id desc OFFSET 0 ROWS FETCH NEXT 1   ROWS ONLY)as messag,(select created_at from messages where recipient_id="+rows.recordset[0].id+" order by id desc OFFSET 0 ROWS FETCH NEXT 1   ROWS ONLY)as created_at FROM trainers_hire  t left join users u on t."+id+"=u.id left join users us on t."+idtrainer+"=us.id   where t.status=1 and u.id="+rows.recordset[0].id;                            
               console.log(qd);
                userModel.query(qd,function(err, rows) {
                    if(err) {
                        res.send({ code: 100,message: 'User not found'});                            
   ...
   if(post) {   
       if( post.id!=='') {
          var userId=post.id;
           userModel = new MyAppModel();
           if(post.action=="1"){
           userModel.query("select u.name,u.email,u.photo,us.name as fromname from trainers_hire th left join users u  on th.user_id=u.id left join users us on th.trainer_id=us.id where    th.id="+userId  , function(err, rows) {
            if (rows.recordset.length<1) {                   
                 res.send({ code: 100,message: 'A valid user is required'});                  
               }
               else { 
                           var q = "UPDATE trainers_hire SET status = '1' WHERE id='" +userId+"'";   
                               ...
                               if(err) {
                                ...
                                emailHelper.sendAccept(newUser, function(err, info  ){
                                    res.send({ code: 200,status: 'accepted',name:newUser.name});  
                                });
                                 //  res.send({ code: 200,message: 'Success'});                                        
                               }
                           });    
